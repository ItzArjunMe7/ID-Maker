...
const IDCard: React.FC<IDCardProps> = ({ data, scale = 1 }) => {
    ...
    <div id={id} className="bg-white rounded-xl shadow-2xl overflow-hidden flex flex-col h-full border border-gray-200 w-[320px] min-h-[500px]">
      <div className="bg-blue-100 p-2 border-b border-blue-200 relative flex items-center h-[90px]">
        <div className="flex-shrink-0 mr-1 ml-4">
           <img src={COLLEGE_LOGO} alt="Logo" className="w-16 h-16 object-contain rounded-full bg-white/50 p-0.5 shadow-sm" />
        </div>
        <div className="flex-1 text-center pr-2">
          <p className="text-[10px] font-bold text-blue-800 tracking-tight leading-none mb-0.5 uppercase">{COLLEGE_INFO.trustName}</p>
          <p className="text-[14px] font-extrabold text-red-600 leading-tight mb-0.5">{COLLEGE_INFO.campusName}</p>
      ...
      <div className="p-4 flex flex-col flex-1 items-center text-center overflow-hidden">
        <h1 className="text-[13px] font-extrabold text-blue-700 leading-tight uppercase mb-0.5 drop-shadow-sm line-clamp-2">{COLLEGE_INFO.collegeName}</h1>
        <p className="text-[7px] text-gray-500 leading-[1.1] mb-2 px-4 italic line-clamp-2">{COLLEGE_INFO.accreditation}</p>
        <div className="flex justify-center w-full mb-3">
          <p className="text-[11px] font-bold text-gray-700 bg-gray-100 px-3 py-0.5 rounded-full border border-gray-200">{COLLEGE_INFO.academicYear}</p>
        </div>
        ...
        <div className="space-y-3 text-sm flex-1 overflow-hidden">
          <div className="flex flex-col">
            ...
            <div className="mt-0.5">
              <p className="text-gray-800 font-medium text-[10px] leading-[1.3] line-clamp-4 break-words">
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <button onClick={() => setIsFront(!isFront)} className="mt-4 no-print px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-full text-sm font-bold transition-all shadow-md transform active:scale-95">Flip Card</button>
    </div>
  );
};
